...
import com.stfe.optim.excel.*;
    ...
    public class testOptimSolveCases {
		...
		public void ojbfunc_optimize() 	{
			...
			double [][] jBestand = restoreIOBean.restoreABeanFromMeta("Meta-BeanArchSpecs-rv_BM_PLAN_CURRENTQUARTER_F","rv_DataF_BM_PLAN_CURRENTQUARTER");
			
			//double [][]bestand = restore.deserializeBeanAsPdouble("rv_Data_BM_SWAPS_PLAN_CURRENTQUARTER",filepath);
			String [][]jBestandDate = (String[][])restore.deserializeBeanAsString("rv_DateF_BM_PLAN_CURRENTQUARTER",filepath);			
			String [][]jBestandEle = (String[][])restore.deserializeBeanAsString("rv_DataPortF_BM_PLAN_CURRENTQUARTER",filepath); //324 rows
			...
			double [][] jBestandEff = com.stfe.optim.util.optimSliceArray.slice2DArray(jBestand, 132, -1, 5, 35 );	
			int numDateRows=jBestandEff.length ;
			
			
			oxl.genTxtFileFrom2DData(filepath, "jBestandEff_CurrentPlanQuarter_DebtClean_id_132_All.txt", jBestandEff);		 
			...
			oxl.genTxtFileFrom2DData(filepath, "swapsRVDataEff-H-AL-5-35_first0.txt", swapsRVDataEff[0]);
			oxl.genTxtFileFrom2DData(filepath, "swapsRVDataEff-H-AL-5-35_last191.txt", swapsRVDataEff[191]);
			...
			double [] upConstraintBase = { 10.00,	50.00,	50.00,	0.20,	0.25,	0.25,	0.50};
			double [] downConstraintBase = { -10.00,	-50.00,	-50.00,	-0.20,	-0.25,	-0.25,	-0.50};
			...
			//jUPConst3D[0][0] = com.stfe.optim.util.optimSliceArray.replcate1DArrayIntoBigArray(upConstraintBase, 64);			
			//double [] upConstraints = com.stfe.optim.util.optimSliceArray.replcate1DArrayIntoBigArray(upConstraintBase, 64);
			...
			//jDNConst3D[0][0] = com.stfe.optim.util.optimSliceArray.replcate1DArrayIntoBigArray((downConstraintBase), 64);			
			//double [] downConstraints = com.stfe.optim.util.optimSliceArray.replcate1DArrayIntoBigArray((downConstraintBase), 64);			
			...
			double [] initSolZero = { 1.0, 1, 1, 1, 1,	1,	1 };
			...
			if (flag2d==12)
			{
				...
				*/
				...
				//op.setInitialSolution("decisionVars", new DoubleMatrixND (solInit));
				...
				op.setInputParameter("dvSolution", op.getPrimalSolution("decisionVars"));			
				System.out.println ("**Quarterly for 3 years (2016-2018) Solution: Optimal Objective Function Value:  ** " + 
			...
			if (flag2d==11)
			{
				...
				*/
				...
				op.setInputParameter("SwapsRV2DOpt", new DoubleMatrixND( swapsRVDataffQ2Y )  );				 				
 				...
 				op.setObjectiveFunction("minimize",  "sum ( ( (1/SchuldOpt') * (PCRP1Opt .*  (sum( (decisionVars) *  SwapsRV2DOpt ,1) + BestandOpt )) )* 10000.0 )" );
			...
			if (flag2d==2)
			{
				...
				//if (!optProb.solutionIsOptimal ()) throw new RuntimeException ("An optimal solution was not found");
				if (!op.solutionIsOptimal ()) System.out.println("??? BAD --- An optimal solution was not found! ???  " );				
				else System.out.println ("#GOOD# TRY 2D -----  Solver done the job! - " + op.solutionIsOptimal()   );
			...
			if (flag2d==1)
			{
				...
				*/
				...
				op.addConstraint(" sum (   sqrt( (decisionVars(0,35:41) ^ 2) + 0.0001d) ) <= StockLimit1Base ");
